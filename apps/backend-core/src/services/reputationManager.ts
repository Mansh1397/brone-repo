...
import { pool } from "./nullifierRegistry";

const { subtle } = webcrypto;

export interface LedgerEntry {
  reputation_key: string;
  metric_type: "posts" | "verifications" | "rewards";
  delta_value: number;
}

const mockLedger: LedgerEntry[] = [];
const mockNonces = new Set<string>();
const queue: LedgerEntry[] = [];
...
async function ensureSchema(): Promise<void> {
  if (schemaInitialized || !process.env.DATABASE_URL || useMemoryDb) return;
  let client: any = null;
  try {
...
export async function verifyReputationSignature(
...
export async function checkAndRecordNonce(nonce: string): Promise<void> {
...
export async function queueReputationDelta(
...
export async function flushQueue(): Promise<void> {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO reputation_ledger_entries (reputation_key, metric_type, delta_value) VALUES ${valuePlaceholders}`,
      values
    );
...
export async function getReputation(reputationKey: string) {
  if (useMemoryDb || !process.env.DATABASE_URL) {
  ...
  try {
    ...
    const res = await client.query(
      "SELECT total_posts, total_verifications, rewards_balance FROM reputation_aggregates WHERE reputation_key = $1",
      [reputationKey]
    );
...
export function startQuantizedQueue(intervalMs = 30000): void {
...
export function stopQuantizedQueue(): void {
  if (intervalId) {
...
export async function resetReputationDatabase(): Promise<void> {
  mockLedger.length = 0;
  mockNonces.clear();
  queue.length = 0;
  useMemoryDb = false;
