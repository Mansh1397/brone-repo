...
interface HardwareRingBundle {
    systemRing: string[];
    hardwareOpaqueKey: string;
}
 ...
 */
export async function fetchHardwareBackedRingAndKey(macroRegionCellId: string): Promise<HardwareRingBundle> {
    try {
        // 1. Fetch the targeted region cell's public signature pool from our backend cluster
        const response = await fetch(`${getBackendUrl()}/api/v1/pools/${macroRegionCellId}`)
            .catch(() => null);

        let systemRing: string[] = [];
        ...
        if (NativeModules.HardwareCryptoBridge?.getOpaqueKeyHandle) {
            hardwareOpaqueKey = await NativeModules.HardwareCryptoBridge.getOpaqueKeyHandle(macroRegionCellId);
        }
...
export function generateRingSignature(
    message: string,
    ring: string[],
    signerPrivateKeyOrHandle: string
