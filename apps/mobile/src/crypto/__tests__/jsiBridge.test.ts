...
function runMockWorker(action: string, args: any): Promise<string> {
  return new Promise((resolve, reject) => {
    const worker = new Worker(MOCK_WORKER_SCRIPT, { eval: true });
...
import { asyncDeriveSharedSecret } from "../ephemeralKeys";
...
describe("Mobile JSI Bridge Performance & Thread Isolation Tests", () => {
  it("should execute 50 high-stress operations concurrently without blocking the main event loop thread", async () => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      promises.push(asyncBlindMessage(rawMessage, blindingFactor, { e, n }));

      // 2. Async ECIES Key Agreement
      const ephemeralPrivKey = BigInt(54321 + i);
      const ephemeralPubKey = BigInt(99887766n + BigInt(i));
      promises.push(asyncDeriveSharedSecret(ephemeralPubKey, ephemeralPrivKey, n));
    }
    ...
    console.log(
      `Main thread event loop registered ${mainThreadTicks} ticks during ${duration}ms of cryptography execution.`
    );
