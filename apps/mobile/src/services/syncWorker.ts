...
import { unblindSignedVoucher } from "../wallet/voucherStripper";
...
let currentActiveToken: TokenOutboxEntry | null = null;
let currentBlindingFactorR: string | null = null;

export function resetSyncWorkerState() {
...
function purgeForensicVariables() {
  ...
  try {
    ...
    (cryptoBroker as any).zeroOutMemory();
  } catch (e) {}
}
 ...
 */
async function getSecureRandomNumber(): Promise<number> {
  try {
    let seedStr = await SecureStore.getItemAsync("sync_worker_secure_seed");
    if (!seedStr) {
      ...
      await SecureStore.setItemAsync("sync_worker_secure_seed", seedStr);
    }
    ...
    await SecureStore.setItemAsync("sync_worker_secure_seed", nextSeed.toString(16));
 ...
 */
export function padPayload(payload: any, targetSize = 2048): string {
 ...
 */
async function syncSingleRecord(db: any, token: TokenOutboxEntry, backendUrl: string): Promise<void> {
  ...
  try {
    ...
    const freshEntry = await getOutboxEntry(token.id);
    ...
    if (token.state === "PENDING_BLINDING") {
      ...
      await db.runAsync(
        "UPDATE local_token_outbox SET state = 'REDEMPTION_SENT', retry_count = retry_count + 1, last_attempted_at = ? WHERE id = ?",
        [Date.now(), token.id]
      );
    }
    ...
    if (token.state === "PENDING_BLINDING") {
      ...
      const unblindedSignature = unblindSignedVoucher(signedBlindedToken, blindFactorR, "v1");
      ...
      await db.runAsync(
        `UPDATE local_token_outbox 
         SET state = 'UNBLINDED', 
             ...
             last_attempted_at = ? 
         WHERE id = ?`,
        [data.signed_blinded_token, unblindedSignature.toString(), Date.now(), token.id]
      );
    } else {
      await db.runAsync(
 ...
 */
export async function runSyncLoop(): Promise<boolean> {
  const backendUrl = process.env.API_GATEWAY_URL || getBackendUrl();
  ...
  try {
    ...
    let pending = await getPendingTokens();
    if (pending.length === 0) {
      return false;
    }
