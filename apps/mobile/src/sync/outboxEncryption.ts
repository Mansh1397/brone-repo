...
export async function getOrCreateOutboxKey(): Promise<string> {
  let key = await SecureStore.getItemAsync("outbox_aes_key");
  if (!key) {
    ...
    await SecureStore.setItemAsync("outbox_aes_key", key);
  }
  return key;
}
...
export function decryptPayload(encryptedStr: string, key: string): string {
