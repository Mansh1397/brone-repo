...
class SyncManager {
    ...
    baseIntervalMs = 10 * 60 * 1000; // 10 minutes
    jitterRangeMs = 5 * 60 * 1000; // 5 minutes
    isSyncing = false;
    constructor() { }
    static getInstance() {
     ...
     */
    async reconcileOfflineOutbox(backendUrl) {
        if (this.isSyncing)
            return 0;
        this.isSyncing = true;
        try {
    ...
    async dispatchReconciledPayload(op, backendUrl) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (op.operationType === "VOTE" || op.operationType === "SUBMISSION") {
                    try {
                        rollbackGuard_1.rollbackGuard.commitVoucherTransaction(op.id, result.receipt_signature || "reconciled-mock-sig");
                    }
                    catch (e) {
                        // Suppress context errors if not staged in active memory
                    }
                }
                await (0, offlineOutbox_1.deleteOfflineOperation)(op.id);
                return true;
            }
            else {
                // HTTP Fail: lock in PENDING_RETRY flag status
                rollbackGuard_1.rollbackGuard.triggerRollbackToPendingRetry(op.id);
                return false;
            }
        }
        catch (err) {
...
exports.syncManager = SyncManager.getInstance();
